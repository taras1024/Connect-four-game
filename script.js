document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0, len = squares.length; i < len; i++)
        ...
        (function (index) {
            //add onclick yo each square in a grid
            squares[i].onclick = function () {
                //if the square below current is taken, you can go on top of it
                if (squares[index + 7].classList.contains('taken')) {
                    ...
                    //if the square below curent is not taken, you can't go here
    ...
    function checkBoard() {
        //const that shows all winning Arrays
        const winningArrays = [
